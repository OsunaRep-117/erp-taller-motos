...
class MockIntegrationHarness {
  ...
  late final ordenRepo = OrdenTrabajoRepositoryImpl(MockOrdenTrabajoDatasource(store));
  late final inventarioRepo = InventarioRepositoryImpl(MockInventarioDatasource(store));
  late final finanzasRepo = FinanzasRepositoryImpl(MockFinanzasDatasource(store));
  ...
  late final crearOrden = CrearOrdenTrabajo(ordenRepo);
  late final asignarMecanico = AsignarMecanico(ordenRepo);
  late final solicitarRefaccion = SolicitarRefaccionParaOrden(inventarioRepo);
  late final terminarOrden = TerminarOrden(
    ordenRepo,
    ConfirmarSalidaInventario(inventarioRepo),
  );
  ...
  Future<Usuario> loginAdmin() async {
    final result = await iniciarSesion(email: 'admin@taller.com', password: 'admin123');
    return result.fold((f) => throw Exception(f.mensaje), (u) => u);
  }

  Future<Usuario> loginRecepcion() async {
    final result = await iniciarSesion(email: 'recep@taller.com', password: 'recep123');
    return result.fold((f) => throw Exception(f.mensaje), (u) => u);
  }
