...
@riverpod
SolicitarRefaccionParaOrden solicitarRefaccionUseCase(SolicitarRefaccionUseCaseRef ref) {
  return SolicitarRefaccionParaOrden(ref.watch(inventarioRepositoryProvider));
}

@riverpod
ConfirmarSalidaInventario confirmarSalidaUseCase(ConfirmarSalidaUseCaseRef ref) {
  return ConfirmarSalidaInventario(ref.watch(inventarioRepositoryProvider));
}

@riverpod
AjustarInventarioManual ajustarInventarioUseCase(AjustarInventarioUseCaseRef ref) {
...
Future<List<Refaccion>> refaccionesDisponibles(RefaccionesDisponiblesRef ref) {
  return ref.watch(inventarioRepositoryProvider).listarRefacciones().then(
...
Future<Refaccion> refaccionPorSku(RefaccionPorSkuRef ref, String sku) {
  return ref.watch(inventarioRepositoryProvider).obtenerPorSku(sku).then(
        (result) => result.fold((f) => throw Exception(f.mensaje), (r) => r),
      );
}
