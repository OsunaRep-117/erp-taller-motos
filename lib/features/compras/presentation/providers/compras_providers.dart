...
Future<List<Proveedor>> proveedoresDisponibles(ProveedoresDisponiblesRef ref) {
  return ref.watch(comprasRepositoryProvider).listarProveedores().then(
        (result) => result.fold((f) => throw Exception(f.mensaje), (r) => r),
      );
}

@riverpod
Future<List<Map<String, dynamic>>> entradasInventario(EntradasInventarioRef ref) {
  return ref.watch(comprasRepositoryProvider).listarEntradas().then(
        (result) => result.fold((f) => throw Exception(f.mensaje), (r) => r),
      );
}
