...
class FinanzasRemoteDatasource implements FinanzasDataSource {
  ...
  Future<void> registrarPago({
    ...
    await client.rpc('registrar_pago', params: {
      'p_id_orden': idOrden,
      'p_monto': monto,
  ...
  Future<List<Pago>> listarTodosLosPagos() async {
    final data = await client.from('pagos').select().order('fecha_pago', ascending: false);
    return data.map(_pagoFromJson).toList();
  }
  ...
  Future<List<Factura>> listarFacturas() async {
    final data = await client.from('facturas').select().order('fecha_emision', ascending: false);
  ...
  Future<void> generarNotaCredito({
    ...
    await client.from('facturas').update({'estado': 'cancelada'}).eq('id', idFactura);
  }
  ...
  Future<double> obtenerIngresosMensuales() async {
    ...
    final data = await client
        ...
        .gte('fecha_pago', DateTime(anioActual, mesActual, 1).toIso8601String());
        
    final total = (data as List).fold<double>(0, (sum, item) => sum + (item['monto'] as num).toDouble());
    return total;
  }

  Future<double> obtenerValorInventario() async {
    final data = await client.from('refacciones').select('stock_actual, precio_costo').eq('inactivo', false);
    final total = (data as List).fold<double>(
        0, (sum, item) => sum + ((item['stock_actual'] as num) * (item['precio_costo'] as num)).toDouble());
    return total;
  }
}
